...
To simply run all workflows in the workspace, use the `Run All Workflows` inline action on the `Workflows` view. This will start up individual VS Code tasks for each workflow defined in your `.github/workflows` folder and create separate entries in the <a href="/docs/usage/history">History</a> view.
...
If you would like to run just a single workflow, locate the workflow in the `Workflows` view and use the `Run Workflow` inline action. This action behaves similar to the previous one, except it will only start a VS Code task and create a history entry for the selected workflow.
...
To run a specific job in a workflow, expand the workflow with the desired job in the `Workflows` view and use the `Run Job` inline action. This action will also only create a single VS Code task and one history entry for the executing job.
...
GitHub events are used to configure workflows to run when specific activities happen on GitHub. These same events can be simulated to run multiple workflows locally. To run an event, use the `Run Event` inline action on the `Workflows` view which will prompt you for the event to trigger. Individual VS Code tasks will be started and history entries will be created for each workflow in the workspace that is set to be triggered by that event.
...
When running jobs, in some cases your workflows may rely on event properties. In this case, you will have to provide these properties as an event payload.
...
Once you have created this file, you can use the `Event Payload` heading under the <a href="/docs/usage/settings">Settings</a> view to include it during workflow execution. This will cause the `act` command to use the `--eventpath` option which specifies the path to the event payload file.

```sh
act --workflows ".github/workflows/application.yaml" --secret-file "" --var-file "" --input-file "" --eventpath "<event_payload_file_path>"
```
